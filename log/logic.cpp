...
bool check_num(int x,int y,int value,int a[9][9]);	//this function checks if "value" can be written in position x,y in the 2d array or not
...
while((count_free!=0)&&(write_value==1)){	//while there are  still empty cells in my sudoku and a value has been assigend to a cell(sudoku can be solved),I want to go through the sudoku again
	...
	for(int i=0;i<9;i++){
		for(int j=0;j<9;j++){	//checking all the cells in the sudoku
			...
			int temp=0;
			for(int k=1;k<10;k++){
				if(check_num(i,j,k,a)==1){	//checking if numbers 1 to 9 can be assigned to that cell
...
bool check_num(int x,int y,int value,int a[9][9]){	//check function to see if that "value" can be written in the  x and y position
